...
const fixtureRoot = path.join(repoRoot, "tests", "fixtures", "no-effect-side-effect-wrapper");
...
function lintWithRule(fixtureFile: string) {
  ...
  const rulePath = path.join(repoRoot, "rules", "no-effect-side-effect-wrapper.grit");
...
describe("no-effect-side-effect-wrapper", () => {
  it("It catches direct Effect.as and Effect.zipRight wrappers around obvious side effects", () => {
    const result = lintWithRule(
      path.join(fixtureRoot, "invalid-direct-side-effect-wrapper.ts"),
    );
  ...
  it("It allows named Effect sequencing without guessing side effects from variable names", () => {
    const result = lintWithRule(path.join(fixtureRoot, "valid-named-effect-sequencing.ts"));
